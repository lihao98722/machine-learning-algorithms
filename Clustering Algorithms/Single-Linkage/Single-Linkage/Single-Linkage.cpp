...
//单连接算法
/*
单连接（SingleLinkage）即nearest-neighbor ，就是取两个类中距离
最近的两个样本的距离作为这两个集合的距离，也就是说，最近两个
样本之间的距离越小，这两个类之间的相似度就越大。
...
*/
void SingleLinkage(vector<Record*> &currentRecords){ //一定要传引用，否则递归过程并没有改变currentRecords
