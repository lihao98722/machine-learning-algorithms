...
//平均连接算法
/*
平均连接算法（AverageLinkage ，就是取两个类中所有
item的平均距离作为这两个集合的距离。
...
*/
void AverageLinkage(vector<Record*> &currentRecords){ //一定要传引用，否则递归过程并没有改变currentRecords
	...
	/*
	每次合并类就同步更新类的信息，并把被合并的类移除，加入到新类的partners容器中。
	在算新的平均距离时，因为本来类中distances里保存着就是此类相对于其余类的平均
	距离，所以只需分别算出总距离(类的个数 *　distances[i])，然后相加得到总距离再除以
	新类中item的个数(两各要合并的类的item个数相加得到)即可得到新类相对于其余类的平均距离。
	*/
	...
	if(minDistance <= threshold){ //小于阀值则合并类
		...
		*/
		...
		for(int i = 0; i < currentRecords[index1]->distances.size(); i++){
			double sum1 = currentRecords[index1]->distances[i] *   
				(currentRecords[index1]->partners.size() + 1);
			double sum2 = currentRecords[index2]->distances[i] * 
				(currentRecords[index2]->partners.size() + 1);
			double count = currentRecords[index1]->partners.size() +   //新类中item个数
				currentRecords[index2]->partners.size() + 2;
			currentRecords[index1]->distances[i] = (sum1 + sum2) / count;  //更新平均距离
		}
